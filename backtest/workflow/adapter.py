"""
Backtest Workflow Adapter — core implementation
=================================================
...
by the extract-workflow-adapter-core-and-shim change
(docs/refactor_program_plan.md Phase 3, step 22 — the finale of the
workflow_adapter decomposition program). `backtest/workflow_adapter.py`
is now a thin compatibility shim that re-exports `BacktestWorkflowAdapter`
and `create_workflow_adapter` from here (plus the six other Phase-3
extractions it already re-exported), so every existing
`from backtest.workflow_adapter import <Name>` import and
`monkeypatch.setattr("backtest.workflow_adapter.<Name>...", ...)` string
path keeps resolving against the same objects.
"""
...
from backtest.workflow.decisions import BacktestDecision
from backtest.workflow.phase1_artifact import (
