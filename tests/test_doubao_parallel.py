"""
Parallel test for Doubao model compatibility
Uses concurrent.futures to run tests in parallel for faster execution
"""
...
class DoubaoParallelTester:
    """Parallel test runner for Doubao compatibility"""
    ...
    def test_portfolio_decision(self) -> TestResult:
        """Test 5: Portfolio manager decision"""
        start_time = time.time()
        try:
            prompt = """
...
def main():
    ...
    print("\n" + "="*70)
    print("🧪 Doubao Parallel Compatibility Test Suite")
    print("="*70)

    tester = DoubaoParallelTester(api_key)
    ...
    # Run parallel
    par_results = tester.run_parallel(max_workers=4)
    print_summary(par_results, "Parallel (4 workers)")
    ...
    if seq_success == len(seq_results) and par_success == len(par_results):
        print("\n🎉 All tests passed! Doubao model is compatible with parallel execution.")
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Parallel test for Doubao compatibility")
