...
from deepear.src.tools.toolkits import NewsToolkit, StockToolkit, SentimentToolkit, SearchToolkit, PolymarketToolkit
from deepear.src.prompts.trend_agent import (
...
class TrendAgent:
    ...
    def __init__(self, db: DatabaseManager, model: Model, tool_model: Optional[Model] = None, sentiment_mode: Optional[str] = None, skip_polymarket: bool = False):
        """
        初始化趋势挖掘 Agent。
        ...
        Args:
            ...
            skip_polymarket: 是否跳过 Polymarket 工具（回测场景建议跳过，网络不可达）
        """
        self.db = db
        self.model = model
        ...
        self.polymarket_toolkit = PolymarketToolkit(db) if not skip_polymarket else None
        
        skip_note = " (skip_polymarket=True)" if skip_polymarket else ""
        logger.info(f"🔧 TrendAgent initialized (Dual-Model: Reasoning={self.model.id}, Tool={self.tool_model.id}{skip_note})")
        ...
        if not skip_polymarket:
            scanner_tools.append(self.polymarket_toolkit)
