"""ReportAgent -- core implementation
=====================================

研报生成器 (ReportAgent)，Map-Reduce 架构，支持增量编辑模式。

Moved verbatim (behavior-preserving) from `deepear/src/agents/report_agent.py`
by the finalize-report-agent-package-and-shim change
(docs/refactor_program_plan.md Phase 4, step 31 -- the finale of the
report_agent decomposition program; every other piece (`retry.py`,
...
earlier Phase 4 steps). `deepear/src/agents/report_agent.py` is now a thin
compatibility shim that re-exports `ReportAgent` from here, so every existing
`from deepear.src.agents.report_agent import ReportAgent` import keeps
resolving to the same class object.

This module -- not `deepear.src.agents.report_agent` -- is now the class's
real home and the canonical monkeypatch seam: `ReportAgent`'s methods
execute in *this* module's namespace, so `Agent`, `ForecastAgent`, and the
`_*_impl` delegation targets (e.g. `_cluster_signals_impl`) must be patched
here (`deepear.src.agents.report.agent.Agent`, etc.), not on the shim module,
for the patch to intercept real method calls. See
`tests/report_agent_harness.py` and
`openspec/changes/finalize-report-agent-package-and-shim/design.md` for the
full rationale.
"""
