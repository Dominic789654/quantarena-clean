...
from deepear.src.utils.content_extractor import ContentExtractor
from deepear.src.utils.llm.factory import get_model
from deepear.src.utils.hybrid_search import LocalNewsSearch
...
class TavilySearchEngine:
    """Tavily Search API 封装 - AI 优化的搜索引擎"""

    def __init__(self):
        self.api_key = os.getenv("TAVILY_API_KEY", "").strip()
        self.has_api_key = bool(self.api_key)
        if self.has_api_key:
            logger.info("✅ Tavily Search API key configured")
        self._client = None
    ...
    def search(self, query: str, max_results: int = 5, search_depth: str = "basic") -> List[Dict]:
        ...
        try:
            ...
            get_stats().record_api_call('search_tavily', success=True, time_ms=elapsed_ms)

            logger.info(f"✅ Tavily search found {len(results)} results for: {query[:50]}...")
            return results

        except Exception as e:
            elapsed_ms = (time.time() - start_time) * 1000
            get_stats().record_api_call('search_tavily', success=False, time_ms=elapsed_ms)
...
class SearchTools:
    ...
    def __init__(self, db: DatabaseManager):
        ...
        self._tavily_enabled = bool(tavily_api_key)
        ...
        # 如果配置了 Tavily API Key，添加 Tavily 引擎
        if self._tavily_enabled:
            self._engines["tavily"] = TavilySearchEngine()
            logger.info("🚀 Tavily Search engine enabled (TAVILY_API_KEY configured)")

        # 确定默认搜索引擎 - 优先级: tavily > jina > ddg
        if self._tavily_enabled:
            self._default_engine = "tavily"
        elif self._jina_enabled:
            self._default_engine = "jina"
        else:
            self._default_engine = "ddg"
    ...
    def search(self, query: str, engine: str = None, max_results: int = 5, ttl: Optional[int] = None) -> str:
        ...
        Args:
            ...
            engine: 搜索引擎选择。可选值:
                    "tavily" (Tavily Search，需配置 TAVILY_API_KEY，AI优化搜索，推荐),
                    "jina" (Jina Search，需配置 JINA_API_KEY，LLM友好输出),
                    "ddg" (DuckDuckGo，推荐英文/国际搜索),
                    "baidu" (百度，推荐中文/国内搜索),
                    "local" (本地历史新闻搜索，基于向量+BM25)。
                    默认: 若配置了 TAVILY_API_KEY 则使用 "tavily"，否则若配置了 JINA_API_KEY 则使用 "jina"，否则 "ddg"。
        ...
        try:
            ...
            if engine == "tavily":
                # Tavily Search 返回 List[Dict]
                tavily_results = tool.search(query, max_results=max_results)
                results = []
            ...
            # 搜索失败时的降级策略
            if engine == "tavily":
                logger.warning(f"⚠️ Tavily search failed, falling back to jina: {query} ({e})")
                try:
                    return self.search(query, engine="jina", max_results=max_results, ttl=ttl)
                except Exception as e2:
                    logger.warning(f"⚠️ Jina fallback also failed, trying ddg: {e2}")
                    try:
                        return self.search(query, engine="ddg", max_results=max_results, ttl=ttl)
                    except Exception as e3:
                        logger.error(f"❌ All fallbacks failed for {query}: {e3}")
            elif engine == "jina":
    ...
    def search_list(self, query: str, engine: str = None, max_results: int = 5, ttl: Optional[int] = None, enrich: bool = True) -> List[Dict]:
        ...
        try:
            ...
            # 注意：如果使用 Tavily 或 Jina Search，内容已经是 LLM 友好格式，可选择跳过 enrichment
            skip_content_enrichment = (engine in ["tavily", "jina"])
